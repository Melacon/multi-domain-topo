# 5GCore / UPF — YANG Models

Augments `ManagedElement` with `UPFFunction`: user-plane packet routing and forwarding on N3 (gNB-UPF), N6 (UPF-DN), and N9 (UPF-UPF) interfaces

This folder models a **disaggregated, containerized UPF deployment**: only the UPF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `UPF`-specific objects hang beneath it |
...
## 3GPP UPF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-upffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `UPFFunction`: user-plane packet routing and forwarding on N3 (gNB-UPF), N6 (UPF-DN), and N9 (UPF-UPF) interfaces |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/UPF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `UPFFunction`
