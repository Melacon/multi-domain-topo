# 5GCore / UDSF — YANG Models

Augments `ManagedElement` with `UDSFFunction`: unstructured data storage for NF-specific state as opaque blobs; NFs use Nudsf API to store and retrieve context data

This folder models a **disaggregated, containerized UDSF deployment**: only the UDSF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `UDSF`-specific objects hang beneath it |
...
## 3GPP UDSF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-udsffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `UDSFFunction`: unstructured data storage for NF-specific state as opaque blobs; NFs use Nudsf API to store and retrieve context data |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/UDSF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `UDSFFunction`
