# 5GCore / SMF — YANG Models

Augments `ManagedElement` with `SMFFunction`: PDU session establishment, IP address allocation, UPF selection via N4, and N11 interface to AMF

This folder models a **disaggregated, containerized SMF deployment**: only the SMF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `SMF`-specific objects hang beneath it |
...
## 3GPP SMF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-smffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `SMFFunction`: PDU session establishment, IP address allocation, UPF selection via N4, and N11 interface to AMF |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/SMF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `SMFFunction`
