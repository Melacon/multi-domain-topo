# 5GCore / N3IWF — YANG Models

Augments `ManagedElement` with `N3IWFFunction`: non-3GPP interworking function that connects untrusted non-3GPP access (e.g. Wi-Fi) to the 5GC via the N2/N3 interfaces

This folder models a **disaggregated, containerized N3IWF deployment**: only the N3IWF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `N3IWF`-specific objects hang beneath it |
...
## 3GPP N3IWF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-n3iwffunction` | 2023-09-18 | 🔀 Augment | Augments `ManagedElement` with `N3IWFFunction`: non-3GPP interworking function that connects untrusted non-3GPP access (e.g. Wi-Fi) to the 5GC via the N2/N3 interfaces |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/N3IWF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `N3IWFFunction`
