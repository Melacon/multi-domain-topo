# 5GCore / NSSF — YANG Models

Augments `ManagedElement` with `NSSFFunction`: NSSAI-based network slice selection for UE sessions; determines which AMF set and NSI serves a given SNSSAI

This folder models a **disaggregated, containerized NSSF deployment**: only the NSSF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `NSSF`-specific objects hang beneath it |
...
## 3GPP NSSF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-nssffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `NSSFFunction`: NSSAI-based network slice selection for UE sessions; determines which AMF set and NSI serves a given SNSSAI |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/NSSF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `NSSFFunction`
