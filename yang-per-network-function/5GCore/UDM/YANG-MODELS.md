# 5GCore / UDM — YANG Models

Augments `ManagedElement` with `UDMFunction`: unified subscriber data management, authentication credential generation, and session management subscription data

This folder models a **disaggregated, containerized UDM deployment**: only the UDM managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `UDM`-specific objects hang beneath it |
...
## 3GPP UDM Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-udmfunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `UDMFunction`: unified subscriber data management, authentication credential generation, and session management subscription data |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/UDM
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `UDMFunction`
