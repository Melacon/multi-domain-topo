# 5GCore / PCF — YANG Models

Augments `ManagedElement` with `PCFFunction`: dynamic QoS and charging policy provisioning; interfaces with SMF (N7), AMF (N15), and AF (N5) for policy rules

This folder models a **disaggregated, containerized PCF deployment**: only the PCF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `PCF`-specific objects hang beneath it |
...
## 3GPP PCF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-pcffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `PCFFunction`: dynamic QoS and charging policy provisioning; interfaces with SMF (N7), AMF (N15), and AF (N5) for policy rules |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/PCF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `PCFFunction`
