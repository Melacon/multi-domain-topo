# 5GCore / AMF — YANG Models

Augments `ManagedElement` with `AMFFunction`: UE registration, mobility, and authentication management; NG-C (N2) and N1 interface configuration; PLMN/SNSSAI serving list

This folder models a **disaggregated, containerized AMF deployment**: only the AMF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `AMF`-specific objects hang beneath it |
...
## 3GPP AMF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-amffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `AMFFunction`: UE registration, mobility, and authentication management; NG-C (N2) and N1 interface configuration; PLMN/SNSSAI serving list |
| `_3gpp-5gc-nrm-nfservice` | 2025-08-18 | 📦 Groupings | NF service instance grouping (service name, version, endpoints) |
| `_3gpp-5gc-nrm-nfprofile` | 2025-08-25 | 📦 Groupings | NF profile grouping (NF type, PLMN, capacity, FQDN); imports nfservice |
| `_3gpp-5gc-nrm-managed-nfprofile` | 2025-11-06 | 📦 Groupings | Managed NF profile extending nfprofile with O1 management attributes |
| `_3gpp-nr-nrm-gnbcuupfunction` | 2024-05-24 | 🔀 Augment | Transitive dep: AMF imports GNBCUUPFunction for UE session handover tracking |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/AMF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `AMFFunction`
