# 5GCore / NEF — YANG Models

Augments `ManagedElement` with `NEFFunction`: network capability exposure to trusted/untrusted AFs via the Nnef API; handles traffic influence and monitoring events

This folder models a **disaggregated, containerized NEF deployment**: only the NEF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `NEF`-specific objects hang beneath it |
...
## 3GPP NEF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-neffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `NEFFunction`: network capability exposure to trusted/untrusted AFs via the Nnef API; handles traffic influence and monitoring events |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/NEF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `NEFFunction`
