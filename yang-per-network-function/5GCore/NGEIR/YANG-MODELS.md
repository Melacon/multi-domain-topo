# 5GCore / NGEIR — YANG Models

Augments `ManagedElement` with `NGEIRFunction`: Next Generation Equipment Identity Register; enforces device blacklists and whitelists for IMEI/IMEISV checks

This folder models a **disaggregated, containerized NGEIR deployment**: only the NGEIR managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `NGEIR`-specific objects hang beneath it |
...
## 3GPP NGEIR Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-ngeirfunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `NGEIRFunction`: Next Generation Equipment Identity Register; enforces device blacklists and whitelists for IMEI/IMEISV checks |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/NGEIR
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `NGEIRFunction`
