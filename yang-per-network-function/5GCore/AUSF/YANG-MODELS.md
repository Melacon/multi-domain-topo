# 5GCore / AUSF — YANG Models

Augments `ManagedElement` with `AUSFFunction`: UE authentication via 5G-AKA and EAP-AKA'; interfaces with UDM (Nausf) and AMF (N12)

This folder models a **disaggregated, containerized AUSF deployment**: only the AUSF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `AUSF`-specific objects hang beneath it |
...
## 3GPP AUSF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-ausffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `AUSFFunction`: UE authentication via 5G-AKA and EAP-AKA'; interfaces with UDM (Nausf) and AMF (N12) |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/AUSF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `AUSFFunction`
