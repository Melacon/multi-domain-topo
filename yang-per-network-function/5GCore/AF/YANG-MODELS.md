# 5GCore / AF — YANG Models

Augments `ManagedElement` with `AFFunction`: a 3GPP-trusted application function that influences traffic routing or QoS via the NEF or directly via the PCF (N5)

This folder models a **disaggregated, containerized AF deployment**: only the AF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `AF`-specific objects hang beneath it |
...
## 3GPP AF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-affunction` | 2023-09-18 | 🔀 Augment | Augments `ManagedElement` with `AFFunction`: a 3GPP-trusted application function that influences traffic routing or QoS via the NEF or directly via the PCF (N5) |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/AF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `AFFunction`
