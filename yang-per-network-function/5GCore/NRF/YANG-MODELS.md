# 5GCore / NRF — YANG Models

Augments `ManagedElement` with `NRFFunction`: NF registration, discovery, and heartbeat management; maintains the NF profile registry used by all SBA consumers

This folder models a **disaggregated, containerized NRF deployment**: only the NRF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `NRF`-specific objects hang beneath it |
...
## 3GPP NRF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-nrffunction` | 2025-08-18 | 🔀 Augment | Augments `ManagedElement` with `NRFFunction`: NF registration, discovery, and heartbeat management; maintains the NF profile registry used by all SBA consumers |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/NRF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `NRFFunction`
