# 5GCore / LMF — YANG Models

Augments `ManagedElement` with `LMFFunction`: UE location management and positioning services; coordinates with NG-RAN and UE for NR positioning measurements

This folder models a **disaggregated, containerized LMF deployment**: only the LMF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `LMF`-specific objects hang beneath it |
...
## 3GPP LMF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-lmffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `LMFFunction`: UE location management and positioning services; coordinates with NG-RAN and UE for NR positioning measurements |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/LMF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `LMFFunction`
