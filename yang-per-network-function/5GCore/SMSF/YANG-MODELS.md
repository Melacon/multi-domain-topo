# 5GCore / SMSF — YANG Models

Augments `ManagedElement` with `SMSFFunction`: SMS delivery over NAS for 5GC subscribers; interfaces with AMF (N20), SMSC (N21), and optionally UDM for SMS subscription data

This folder models a **disaggregated, containerized SMSF deployment**: only the SMSF managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `SMSF`-specific objects hang beneath it |
...
## 3GPP SMSF Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-smsffunction` | 2025-11-06 | 🔀 Augment | Augments `ManagedElement` with `SMSFFunction`: SMS delivery over NAS for 5GC subscribers; interfaces with AMF (N20), SMSC (N21), and optionally UDM for SMS subscription data |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/SMSF
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `SMSFFunction`
