# 5GCore / SEPP — YANG Models

Augments `ManagedElement` with `SEPPFunction`: Security Edge Protection Proxy for inter-PLMN roaming; terminates the N32 interface and applies topology-hiding and message filtering

This folder models a **disaggregated, containerized SEPP deployment**: only the SEPP managed
...
| Module | Role |
|--------|------|
| `_3gpp-common-managed-element` | 🌳 **Root** — `ManagedElement` list; `SEPP`-specific objects hang beneath it |
...
## 3GPP SEPP Function Module

| Module | Revision | Role | Description |
|--------|----------|------|-------------|
| `_3gpp-5gc-nrm-seppfunction` | 2023-09-18 | 🔀 Augment | Augments `ManagedElement` with `SEPPFunction`: Security Edge Protection Proxy for inter-PLMN roaming; terminates the N32 interface and applies topology-hiding and message filtering |
...
```bash
./generate-yang-tree.sh yang-per-network-function/5GCore/SEPP
```

Output written to `yang-tree.txt`. The tree starts at `ManagedElement` with `SEPPFunction`
