...
│   ├── ROADM/
│   └── 5GCore/
│       ├── yang-models/               # Shared YANG model set for all 5GC NFs
│       ├── AF/ … UPF/                 # Per-NF type folders (symlinks → yang-models)
│       └── */ietf-yang-library.json   # One yang library per 5GC NF type
...
├── scripts/
│   └── generate-ietf-yang-library.py  # Generates ietf-yang-library.json per NF type
...
- `ietf-yang-library.json` — RFC 8525 YANG library for this NF type (generated by `scripts/generate-ietf-yang-library.py`)
...
The `ietf-yang-library.json` is a **type-level** artifact: it depends only on which YANG modules define the NF type, not on any particular instance.  For the 5GCore domain each NF function (AF, AMF, UPF, …) has its own `ietf-yang-library.json` inside its NF-type subfolder; all those subfolders share the same physical YANG files via symlinks into `5GCore/yang-models/`.
...
> **O-CU note:** The O-CU is one logical unit (gNB-CU) but comprises two separately managed NFs — O-CU-CP and O-CU-UP — connected via the **E1** interface. In this repository they are modelled as sibling sub-directories under each `O-CU-N/` instance parent. Their YANG module sets and `ietf-yang-library.json` files live in `yang-per-network-function/O-CU-CP/` and `yang-per-network-function/O-CU-UP/` respectively, reflecting the distinct YANG model set for each plane.
...
Per-NF-instance data directories, each containing `ietf-system.json` (RFC 7317 system information) and other instance-specific YANG data and config samples used by the topology engine.  The `ietf-yang-library.json` is **not** stored here — it is a type-level artifact and lives in `yang-per-network-function/<NF-type>/` instead.
